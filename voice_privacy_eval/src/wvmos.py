...
class Wav2Vec2MOS(nn.Module):
    def __init__(self, path=PATH, freeze=True, cuda=True):
        super().__init__()
        path = path
        self.encoder = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-base")
        self.freeze = freeze
        ...
        self.load_state_dict(extract_prefix('model.', torch.load(path, map_location=device, weights_only=False)['state_dict']))
        self.eval()
        self.cuda_flag = cuda
        if cuda:
            self.cuda()
